...
locals {
  ...
  name_prefix             = lower(join("-", compact([var.org, var.environment, var.region_key, "embedding"])))
  ...
  bucket_names = {
    ...
    state  = coalesce(var.state_bucket_name, "${local.name_prefix}-state")
  ...
  event_rule_name  = coalesce(var.event_rule_display_name, "${local.name_prefix}-object-event")
