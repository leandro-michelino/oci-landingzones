...
locals {
  ...
  name_prefix             = lower(join("-", compact([var.org, var.environment, var.region_key, "genai-ft"])))
  target_compartment_ocid = coalesce(var.compartment_ocid, var.tenancy_ocid)
  policy_compartment_ocid = coalesce(var.policy_compartment_ocid, var.tenancy_ocid)
  training_bucket_name    = coalesce(var.training_bucket_name, "${local.name_prefix}-training")
  cluster_display_name    = coalesce(var.cluster_display_name, "${local.name_prefix}-cluster")
  model_display_name      = coalesce(var.model_display_name, "${local.name_prefix}-model")
  endpoint_display_name   = coalesce(var.endpoint_display_name, "${local.name_prefix}-endpoint")
