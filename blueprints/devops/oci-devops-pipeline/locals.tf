...
locals {
  ...
  name_prefix             = lower(join("-", compact([var.org, var.environment, var.region_key, "oci-devops-pipeline"])))
  target_compartment_ocid = coalesce(var.compartment_ocid, var.tenancy_ocid)
  common_freeform_tags = merge(var.freeform_tags, {
    ManagedBy = "Terraform"
    Blueprint = local.blueprint_name
  })
  topic_name            = coalesce(var.topic_name, "${local.name_prefix}-devops-topic")
  notification_topic_id = var.create_notification_topic ? oci_ons_notification_topic.this[0].id : var.notification_topic_id
  project_name          = coalesce(var.project_name, "${local.name_prefix}-devops")
