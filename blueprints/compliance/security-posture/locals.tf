...
locals {
  ...
  name_prefix             = lower(join("-", compact([var.org, var.environment, var.region_key, "secposture"])))
  target_compartment_ocid = coalesce(var.compartment_ocid, var.tenancy_ocid)
  policy_compartment_ocid = coalesce(var.policy_compartment_ocid, var.tenancy_ocid)
  report_bucket_name      = coalesce(var.report_bucket_name, "${local.name_prefix}-reports")
