...
This architecture page documents the `blueprints/compliance/zero-trust` deployment. It is intentionally text-first and ASCII-only so it can be reviewed in terminals, pull requests, and customer notes without a diagramming tool.

## Deployment Purpose

This blueprint provides zero-trust landing zone that composes core security services
with ZPR-aware private network segmentation.
...
For Zero Trust, the important hand-off values are `blueprint_name`, `name_prefix`, `resource_ids`, `root_compartment_id`, `compartment_ids`, `vcn_id`, `subnet_ids`, `zpr_policy_ids`. Keep those names stable unless a downstream blueprint, runbook, or customer hand-off is updated at the same time.
