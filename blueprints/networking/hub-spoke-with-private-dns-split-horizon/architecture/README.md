# Hub-Spoke With Private DNS Split Horizon Architecture

Author: Leandro Michelino | ACE | leandro.michelino@oracle.com

This architecture page documents the `blueprints/networking/hub-spoke-with-private-dns-split-horizon` deployment. It is intentionally text-first and ASCII-only so it can be reviewed in terminals, pull requests, and customer notes without a diagramming tool.

## Deployment Purpose

This blueprint provides networking landing-zone pattern for the hub-spoke with private
dns split horizon.
...
- `module private_dns` in `main.tf`: composes the reusable private dns module or child blueprint.
...
$ terraform output
blueprint_name = "hub-spoke-with-private-dns-split-horizon"
name_prefix = "<org>-<env>-<region_key>"
resource_ids = { ... }
hub_vcn_id = "ocid1.<resource>..."
spoke_vcn_ids = { ... }
private_view_id = "ocid1.<resource>..."
private_zone_ids = { ... }
vcn_resolver_ids = { ... }
```
...
For Hub Spoke With Private DNS Split Horizon, the important hand-off values are `blueprint_name`, `name_prefix`, `resource_ids`, `hub_vcn_id`, `spoke_vcn_ids`, `private_view_id`, `private_zone_ids`, `vcn_resolver_ids`. Keep those names stable unless a downstream blueprint, runbook, or customer hand-off is updated at the same time.
