...
`blueprints/networking/hub-spoke-with-private-dns-split-horizon`. It stays ASCII-first on
...
| Item | Details |
| --- | --- |
| Boundary | `blueprints/networking/hub-spoke-with-private-dns-split-horizon` owns this deployment end to end. |
...
| 02. module.private_dns -> modules/networking/dns @ v0.1.0                                            |
|                                                                                                      |
...
| Module | `private_dns` | `modules/networking/dns @ v0.1.0` |
...
For Hub-Spoke With Private DNS Split Horizon, the important hand-off values are
`blueprint_name`, `name_prefix`, `resource_ids`, `hub_vcn_id`, `spoke_vcn_ids`,
`private_view_id`, `private_zone_ids`, `vcn_resolver_ids`. Keep those names stable unless a
