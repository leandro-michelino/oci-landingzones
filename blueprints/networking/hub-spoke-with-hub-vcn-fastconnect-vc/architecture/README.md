...
This is the design view for `blueprints/networking/hub-spoke-with-hub-vcn-fastconnect-vc`.
...
| Item | Details |
| --- | --- |
| Boundary | `blueprints/networking/hub-spoke-with-hub-vcn-fastconnect-vc` owns this deployment end to end. |
...
| Output contract | `blueprint_name`, `name_prefix`, `resource_ids`, `hub_vcn_id`, `drg_id`, `spoke_vcn_ids`, `virtual_circuit_id`, `virtual_circuit_state` |
...
For Hub-Spoke With FastConnect Virtual Circuit, the important hand-off values are
`blueprint_name`, `name_prefix`, `resource_ids`, `hub_vcn_id`, `drg_id`, `spoke_vcn_ids`,
`virtual_circuit_id`, `virtual_circuit_state`. Keep those names stable unless a downstream
