# Hub-Spoke With FastConnect Virtual Circuit Architecture

Author: Leandro Michelino | ACE | leandro.michelino@oracle.com

This architecture page documents the `blueprints/networking/hub-spoke-with-hub-vcn-fastconnect-vc` deployment. It is intentionally text-first and ASCII-only so it can be reviewed in terminals, pull requests, and customer notes without a diagramming tool.
...
$ terraform output
blueprint_name = "hub-spoke-with-hub-vcn-fastconnect-vc"
...
For Hub Spoke With Hub VCN Fastconnect Vc, the important hand-off values are `blueprint_name`, `name_prefix`, `resource_ids`, `hub_vcn_id`, `drg_id`, `spoke_vcn_ids`, `virtual_circuit_id`, `virtual_circuit_state`. Keep those names stable unless a downstream blueprint, runbook, or customer hand-off is updated at the same time.
