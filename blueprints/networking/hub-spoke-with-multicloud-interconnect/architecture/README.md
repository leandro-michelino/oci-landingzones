...
This is the design view for `blueprints/networking/hub-spoke-with-multicloud-interconnect`.
...
| Item | Details |
| --- | --- |
| Boundary | `blueprints/networking/hub-spoke-with-multicloud-interconnect` owns this deployment end to end. |
...
| Output contract | `blueprint_name`, `name_prefix`, `resource_ids`, `hub_vcn_id`, `drg_id`, `spoke_vcn_ids`, `virtual_circuit_id`, `ipsec_id` |
...
For Hub-Spoke With Multicloud Interconnect, the important hand-off values are
`blueprint_name`, `name_prefix`, `resource_ids`, `hub_vcn_id`, `drg_id`, `spoke_vcn_ids`,
`virtual_circuit_id`, `ipsec_id`. Keep those names stable unless a downstream blueprint,
