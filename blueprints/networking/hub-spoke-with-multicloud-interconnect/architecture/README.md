# Hub-Spoke With Multicloud Interconnect Architecture

Author: Leandro Michelino | ACE | leandro.michelino@oracle.com

This architecture page documents the `blueprints/networking/hub-spoke-with-multicloud-interconnect` deployment. It is intentionally text-first and ASCII-only so it can be reviewed in terminals, pull requests, and customer notes without a diagramming tool.

## Deployment Purpose

This blueprint provides networking landing-zone pattern for the hub-spoke with
...
|  02. module   fastconnect                                      (main.tf)                  |
|  03. module   ipsec_vpn                                        (main.tf)                  |
+-------------------------------------------------------------------------------------------+
|   |                                                                                       |
|   v                                                                                       |
+-------------------------------------------------------------------------------------------+
...
- `module fastconnect` in `main.tf`: composes the reusable fastconnect module or child blueprint.
- `module ipsec_vpn` in `main.tf`: composes the reusable ipsec vpn module or child blueprint.
...
spoke_vcn_ids = { ... }
virtual_circuit_id = "ocid1.<resource>..."
ipsec_id = "ocid1.<resource>..."
```
...
For Hub Spoke With Multicloud Interconnect, the important hand-off values are `blueprint_name`, `name_prefix`, `resource_ids`, `hub_vcn_id`, `drg_id`, `spoke_vcn_ids`, `virtual_circuit_id`, `ipsec_id`. Keep those names stable unless a downstream blueprint, runbook, or customer hand-off is updated at the same time.
