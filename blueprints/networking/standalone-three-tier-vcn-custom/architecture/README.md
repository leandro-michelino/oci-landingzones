...
This architecture page documents the `blueprints/networking/standalone-three-tier-vcn-custom` deployment. It is intentionally text-first and ASCII-only so it can be reviewed in terminals, pull requests, and customer notes without a diagramming tool.
...
$ terraform output
blueprint_name = "standalone-three-tier-vcn-custom"
...
For Standalone Three Tier VCN Custom, the important hand-off values are `blueprint_name`, `name_prefix`, `resource_ids`, `vcn_id`, `subnet_ids`, `route_table_ids`, `gateway_ids`. Keep those names stable unless a downstream blueprint, runbook, or customer hand-off is updated at the same time.
