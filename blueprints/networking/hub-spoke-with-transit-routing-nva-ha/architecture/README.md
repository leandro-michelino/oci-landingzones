# Hub-Spoke With Transit Routing NVA HA Architecture

Author: Leandro Michelino | ACE | leandro.michelino@oracle.com

This architecture page documents the `blueprints/networking/hub-spoke-with-transit-routing-nva-ha` deployment. It is intentionally text-first and ASCII-only so it can be reviewed in terminals, pull requests, and customer notes without a diagramming tool.

## Deployment Purpose

This blueprint provides networking landing-zone pattern for the hub-spoke with transit
routing nva ha.
...
- `module nva_ha` in `main.tf`: composes the reusable nva ha module or child blueprint.
...
$ terraform output
blueprint_name = "hub-spoke-with-transit-routing-nva-ha"
...
For Hub Spoke With Transit Routing NVA Ha, the important hand-off values are `blueprint_name`, `name_prefix`, `resource_ids`, `hub_vcn_id`, `drg_id`, `spoke_vcn_ids`, `appliance_instance_ids`, `route_target_private_ip_ids`. Keep those names stable unless a downstream blueprint, runbook, or customer hand-off is updated at the same time.
