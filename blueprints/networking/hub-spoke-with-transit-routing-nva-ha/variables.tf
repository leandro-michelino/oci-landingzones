...
variable "enable_reserved_route_ips" {
  description = "Create reserved private IPs for HA route targets."
  type        = bool
  default     = false
}

variable "appliances" {
  description = "HA NVA instances keyed by logical name."
  type = map(object({
...
variable "reserved_route_ips" {
  description = "Reserved private IP route targets keyed by logical HA node."
  type = map(object({
    subnet_id      = string
...
variable "existing_route_target_private_ip_ids" {
  description = "Existing NVA private IP OCIDs keyed by HA node."
  type        = map(string)
  default     = {}
}
