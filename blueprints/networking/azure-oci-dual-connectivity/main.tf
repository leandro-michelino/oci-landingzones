...
resource "oci_core_route_table" "primary" {
  ...
  dynamic "route_rules" {
    for_each = var.enable_ipsec_fallback ? var.azure_network_cidrs : []
...
resource "oci_ons_notification_topic" "connectivity_alert" {
  count = var.enable_connectivity_alert_topic ? 1 : 0

  compartment_id = local.target_compartment_ocid
  name           = local.connectivity_topic_name
  description    = "Azure + OCI dual connectivity alerts for ${local.name_prefix}."
  defined_tags   = var.defined_tags
  freeform_tags  = local.common_freeform_tags
}
