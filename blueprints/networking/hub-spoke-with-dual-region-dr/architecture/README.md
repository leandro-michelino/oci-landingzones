...
This pattern models primary and secondary hub-spoke stacks across two regions with remote peering, DNS behavior, routing, and failover paths.
...
| Hub VCN + DRG             | <----> | Hub VCN + DRG             |
| Spoke VCNs                | RPC    | Standby Spoke VCNs        |
+---------------------------+        +---------------------------+
        |                                      |
        v                                      v
 Primary DNS / LB                       Standby DNS / LB
        |                                      ^
        v                                      |
 Application traffic              Failover route and DNS change
        |                                      |
