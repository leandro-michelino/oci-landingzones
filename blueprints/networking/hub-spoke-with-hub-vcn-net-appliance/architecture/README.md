...
This page is the deployment architecture for `blueprints/networking/hub-spoke-with-hub-vcn-net-appliance`. It is intentionally ASCII-first so it
...
- Confirm `terraform output` will expose the hand-off values expected by downstream teams: `blueprint_name`, `name_prefix`, `resource_ids`, `hub_vcn_id`, `drg_id`, `hub_subnet_ids`, `spoke_vcn_ids`, `route_target_private_ip_ids`.
...
This is the expected close-out shape for `blueprints/networking/hub-spoke-with-hub-vcn-net-appliance`. Terraform owns the OCI resource graph and
