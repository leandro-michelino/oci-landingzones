...
This architecture page documents the `blueprints/networking/multi-tenancy-shared-services` deployment. It is intentionally text-first and ASCII-only so it can be reviewed in terminals, pull requests, and customer notes without a diagramming tool.

## Deployment Purpose

This blueprint provides networking landing-zone pattern for the multi-tenancy shared
services.
...
- `module shared_dns` in `main.tf`: composes the reusable shared dns module or child blueprint.
...
$ terraform output
blueprint_name = "multi-tenancy-shared-services"
...
For Multi Tenancy Shared Services, the important hand-off values are `blueprint_name`, `name_prefix`, `resource_ids`, `hub_vcn_id`, `spoke_vcn_ids`, `private_view_id`, `private_zone_ids`. Keep those names stable unless a downstream blueprint, runbook, or customer hand-off is updated at the same time.
