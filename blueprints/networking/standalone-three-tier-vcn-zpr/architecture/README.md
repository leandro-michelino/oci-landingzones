# Standalone Three-Tier VCN With ZPR Architecture

Author: Leandro Michelino | ACE | leandro.michelino@oracle.com

This architecture page documents the `blueprints/networking/standalone-three-tier-vcn-zpr` deployment. It is intentionally text-first and ASCII-only so it can be reviewed in terminals, pull requests, and customer notes without a diagramming tool.

## Deployment Purpose

This blueprint provides networking landing-zone pattern for the standalone three-tier
vcn with zpr.
...
|  01. module   workload_vcn                                     (main.tf)                  |
|  02. module   zpr                                              (main.tf)                  |
+-------------------------------------------------------------------------------------------+
|   |                                                                                       |
|   v                                                                                       |
+-------------------------------------------------------------------------------------------+
...
- `module workload_vcn` in `main.tf`: composes the reusable workload vcn module or child blueprint.
- `module zpr` in `main.tf`: composes the reusable zpr module or child blueprint.
...
$ terraform output
blueprint_name = "standalone-three-tier-vcn-zpr"
...
For Standalone Three Tier VCN ZPR, the important hand-off values are `blueprint_name`, `name_prefix`, `resource_ids`, `vcn_id`, `subnet_ids`, `zpr_configuration_id`, `zpr_policy_ids`. Keep those names stable unless a downstream blueprint, runbook, or customer hand-off is updated at the same time.
