# Hub-Spoke With ZPR Micro-Segmentation Architecture

Author: Leandro Michelino | ACE | leandro.michelino@oracle.com

This architecture page documents the `blueprints/networking/hub-spoke-with-zpr-micro-segmentation` deployment. It is intentionally text-first and ASCII-only so it can be reviewed in terminals, pull requests, and customer notes without a diagramming tool.

## Deployment Purpose

This blueprint provides networking landing-zone pattern for the hub-spoke with zpr
micro-segmentation.
...
$ terraform output
blueprint_name = "hub-spoke-with-zpr-micro-segmentation"
...
For Hub Spoke With ZPR Micro Segmentation, the important hand-off values are `blueprint_name`, `name_prefix`, `resource_ids`, `hub_vcn_id`, `spoke_vcn_ids`, `zpr_configuration_id`, `zpr_policy_ids`. Keep those names stable unless a downstream blueprint, runbook, or customer hand-off is updated at the same time.
