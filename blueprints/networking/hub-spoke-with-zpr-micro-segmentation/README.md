...
Start here for `blueprints/networking/hub-spoke-with-zpr-micro-segmentation`: what it builds, which inputs deserve a careful look, how to run Terraform or the local Ansible wrappers, and where the detailed architecture notes live.

## At A Glance

| Item | Details |
| --- | --- |
| Folder | `blueprints/networking/hub-spoke-with-zpr-micro-segmentation` |
| Best fit | Adds ZPR configuration and policies to a hub-spoke network so micro-segmentation is part of the topology. |
| Terraform shape | `network`, `zpr` |
| Inputs to settle first | `compartment_ocid`, `zpr_policies` |
| Outputs to hand off | `blueprint_name`, `name_prefix`, `resource_ids`, `hub_vcn_id`, `spoke_vcn_ids`, `zpr_configuration_id`, `zpr_policy_ids` |
| Local runner | `terraform plan` for quick iteration; `ansible/plan.yml` and guarded `ansible/apply.yml` for the repo-standard flow. |

## Deployment Purpose

Adds ZPR configuration and policies to a hub-spoke network so micro-segmentation is part of
the topology.
...
| Input | What To Decide |
| --- | --- |
| `enable_zpr_configuration` | Enable ZPR configuration for this landing zone. Optional and intentionally off by default. |
| `enable_zpr_policies` | Create ZPR policy statements for micro-segmentation. |
