# Hub-Spoke With Bastion Jump Host Architecture

Author: Leandro Michelino | ACE | leandro.michelino@oracle.com

This is the design view for
`blueprints/networking/hub-spoke-with-hub-vcn-bastion-jump-host`. It stays ASCII-first on
...
| Item | Details |
| --- | --- |
| Boundary | `blueprints/networking/hub-spoke-with-hub-vcn-bastion-jump-host` owns this deployment end to end. |
...
| Output contract | `blueprint_name`, `name_prefix`, `resource_ids`, `hub_vcn_id`, `drg_id`, `hub_subnet_ids`, `spoke_vcn_ids`, `bastion_id`, plus 1 more |
...
| 02. module.bastion -> modules/security/bastion @ v0.1.0                                              |
|                                                                                                      |
...
| Module | `bastion` | `modules/security/bastion @ v0.1.0` |
...
$ terraform output
blueprint_name = "hub-spoke-with-hub-vcn-bastion-jump-host"
...
hub_subnet_ids = { ... }
spoke_vcn_ids = { ... }
bastion_id = "ocid1.<resource>..."
bastion_private_endpoint_ip_address = "<value>"
```
...
For Hub-Spoke With Bastion Jump Host, the important hand-off values are `blueprint_name`,
`name_prefix`, `resource_ids`, `hub_vcn_id`, `drg_id`, `hub_subnet_ids`, `spoke_vcn_ids`,
`bastion_id`, `bastion_private_endpoint_ip_address`. Keep those names stable unless a
