...
This architecture page documents the `blueprints/networking/hub-spoke-with-hub-vcn-bastion-jump-host` deployment. It is intentionally text-first and ASCII-only so it can be reviewed in terminals, pull requests, and customer notes without a diagramming tool.

## Deployment Purpose

This blueprint provides networking landing-zone pattern for the hub-spoke with bastion
jump host.
...
- `module bastion` in `main.tf`: composes the reusable bastion module or child blueprint.
...
bastion_private_endpoint_ip_address = "<private-ip>"
```
