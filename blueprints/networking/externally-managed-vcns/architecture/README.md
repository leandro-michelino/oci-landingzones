...
This pattern integrates VCNs owned outside the repository while keeping managed resources, ownership boundaries, routing, and DNS integration explicit.
...
```text
External Network Owner
          | owns
          v
+----------------------+      +----------------------+
| Existing VCNs        | ---> | Integration Points   |
| CIDRs, subnets       |      | routes, DNS, SGW     |
+----------------------+      +----------------------+
          |                            |
          v                            v
+----------------------+      +----------------------+
| Landing Zone Managed |      | Shared Services      |
| IAM, logging, tags   |      | monitoring, security |
+----------------------+      +----------------------+
          |
          v
Document handoff for every resource not managed by Terraform here
```
