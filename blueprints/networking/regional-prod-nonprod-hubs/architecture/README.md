...
This architecture page documents the `blueprints/networking/regional-prod-nonprod-hubs` deployment. It is intentionally text-first and ASCII-only so it can be reviewed in terminals, pull requests, and customer notes without a diagramming tool.

## Deployment Purpose

This blueprint provides networking landing-zone pattern for the regional prod/nonprod
hubs.
...
|  01. module   prod_network                                     (main.tf)                  |
|  02. module   nonprod_network                                  (main.tf)                  |
+-------------------------------------------------------------------------------------------+
|   |                                                                                       |
|   v                                                                                       |
+-------------------------------------------------------------------------------------------+
...
- `module prod_network` in `main.tf`: composes the reusable prod network module or child blueprint.
- `module nonprod_network` in `main.tf`: composes the reusable nonprod network module or child blueprint.
...
resource_ids = { ... }
prod_hub_vcn_id = "ocid1.<resource>..."
nonprod_hub_vcn_id = "ocid1.<resource>..."
prod_drg_id = "ocid1.<resource>..."
nonprod_drg_id = "ocid1.<resource>..."
```
...
For Regional Prod Nonprod Hubs, the important hand-off values are `blueprint_name`, `name_prefix`, `resource_ids`, `prod_hub_vcn_id`, `nonprod_hub_vcn_id`, `prod_drg_id`, `nonprod_drg_id`. Keep those names stable unless a downstream blueprint, runbook, or customer hand-off is updated at the same time.
