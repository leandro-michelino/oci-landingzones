...
This architecture page documents the `blueprints/cis/level2` deployment. It is intentionally text-first and ASCII-only so it can be reviewed in terminals, pull requests, and customer notes without a diagramming tool.

## Deployment Purpose

This blueprint provides CIS Level 2 wrapper around the core landing-zone controls with
stricter security posture defaults.
