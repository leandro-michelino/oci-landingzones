...
This planned disaster-recovery pattern describes primary and standby regions, DR protection groups, plans, replicated dependencies, DNS or load-balancer flow, networking, monitoring, notifications, and operator touchpoints.
...
```text
                 DR Runbook / Operators
                         |
                         v
+-------------------+     plan      +-------------------+
| Primary Region    | ------------> | Standby Region    |
| app, data, network|               | app, data, network|
+-------------------+               +-------------------+
        |                                     ^
        | replication and dependency checks   |
        v                                     |
+-------------------+               +-------------------+
| DR Protection     |               | DR Protection     |
| Group and plans   |               | Group and plans   |
+-------------------+               +-------------------+
        |                                     |
        v                                     v
DNS / LB failover path          Monitoring and notifications
```
