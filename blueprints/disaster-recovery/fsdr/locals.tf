...
locals {
  ...
  primary_log_bucket_name  = coalesce(var.primary_log_bucket_name, "${local.name_prefix}-bkt-fsdr-logs")
  standby_log_bucket_name  = coalesce(var.standby_log_bucket_name, "${local.standby_name_prefix}-bkt-fsdr-logs")
