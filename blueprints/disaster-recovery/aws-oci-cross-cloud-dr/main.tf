...
resource "terraform_data" "connectivity_contract" {
  ...
  lifecycle {
    precondition {
      ...
      error_message = "For connectivity_mode=interconnect, set both fastconnect_virtual_circuit_id and direct_connect_connection_id. For connectivity_mode=without-interconnect, keep both values null."
    }
  }
}
  ...
  lifecycle {
    precondition {
      condition = (
        ...
        var.aws_standby_endpoint != null &&
        trimspace(var.app_fqdn) != ""
      )
      error_message = "When enable_dns_failover_contract=true, set app_fqdn, oci_primary_endpoint, and aws_standby_endpoint."
    }
  }
}
