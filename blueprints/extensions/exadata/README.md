...
Start here for `blueprints/extensions/exadata`: what it builds, which inputs deserve a careful look, how to run Terraform or the local Ansible wrappers, and where the detailed architecture notes live.
...
For extension-only use, supply existing compartment, availability domain,
maintenance, and network-adjacent values in local tfvars and run this folder
directly. For base-plus-extension use, deploy core and networking first, then
pass their outputs here.
