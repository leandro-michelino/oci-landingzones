...
|                                              `-> Azure endpoint (standby on failover)                    |
|                                                                                                          |
...
- Data plane traffic is the packet or service path shown in the Architecture diagram: client traffic targets OCI primary and shifts to Azure standby only when failover policy health rules require it.
...
- With the default 30-second TTL, 30-second health check interval, and 10-second health check timeout, use 60 to 120 seconds as the practical target for both OCI-to-Azure failover and Azure-to-OCI failback. Measure from the delegated Traffic Management FQDN, not from direct endpoint IPs.
...
- Kubernetes versions should be advanced to the newest common provider-supported minor version during each deployment review. Use the latest version supported by both OKE and AKS in the selected regions; keep the Terraform and Bicep examples at `1.36` where supported, and override only when the target region has not exposed that version yet.
