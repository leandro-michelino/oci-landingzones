# AKS + OKE Active Passive

Start here for `blueprints/extensions/aks-oke-active-passive`: what it builds, which inputs deserve a careful look, how to run Terraform or the local Ansible wrappers, and where the detailed architecture notes live.
...
| Best fit | Active/passive failover AKS + OKE operating model with OCI as primary, partner interconnect, GitOps contract, and DNS failover contract. |
...
| Inputs to settle first | `compartment_ocid`, `enable_oke_networking`, `oke_vcn_cidr`, `oke_node_subnet_cidrs`, `oke_kubernetes_version`, `fastconnect_virtual_circuit_id`, `expressroute_circuit_id`, `aks_cluster_id`, `app_fqdn`, `oci_primary_endpoint`, `azure_secondary_endpoint`, `enable_oci_traffic_management`, `oci_traffic_management_zone_id` |
...
Implements an OCI-primary AKS + OKE active/passive Kubernetes pattern that uses
...
Use the latest Kubernetes minor version supported by both OKE and AKS in the
...
is supported by the selected cloud regions. If one provider or region has not
exposed that version yet, pin both clusters to the newest common supported
minor version for the E2E run.
...
| Resource | `terraform_data.traffic_steering_contract` | Active/passive DNS failover metadata for OCI primary and Azure standby. |
...
| Optional resource | `oci_dns_steering_policy.traffic_failover` | OCI DNS Traffic Management failover policy with OCI as primary and Azure as standby. |
...
```text
blueprints/extensions/aks-oke-active-passive/
...
| `oke_kubernetes_version` | Kubernetes version for OKE resources. Use the latest common minor version supported by both OKE and AKS. |
...
| `secondary_cluster` | AKS standby cluster IDs and cloud role metadata. |
...
| `dns_failover_contract` | DNS failover contract for OCI primary to Azure standby cutover, including OCI Traffic Management resource IDs when enabled. |
...
endpoint and serves the Azure standby answer only when the failover policy marks
...
on the OKE primary endpoint and `Hello World Azure` on the AKS standby endpoint.
...
target of 60 to 120 seconds for OCI-to-Azure failover and Azure-to-OCI failback
...
`Hello World OCI` to `Hello World Azure` during failover, then from
`Hello World Azure` back to `Hello World OCI` during failback. Direct IP tests
