...
This architecture page documents the `blueprints/extensions/oke` deployment. It is intentionally text-first and ASCII-only so it can be reviewed in terminals, pull requests, and customer notes without a diagramming tool.

## Deployment Purpose

This blueprint provides optional OKE cluster and node-pool extension for an existing
network foundation.
...
|  01. resource oci_containerengine_cluster.this                 (main.tf)                     |
|  02. resource oci_containerengine_node_pool.this               (main.tf)                     |
+----------------------------------------------------------------------------------------------+
|   |                                                                                          |
|   v                                                                                          |
+----------------------------------------------------------------------------------------------+
...
- `resource oci_containerengine_cluster.this` in `main.tf`: creates an OKE Kubernetes control plane.
- `resource oci_containerengine_node_pool.this` in `main.tf`: creates an OKE worker node pool.
...
For OKE, the important hand-off values are `blueprint_name`, `name_prefix`, `resource_ids`, `cluster_id`, `node_pool_id`. Keep those names stable unless a downstream blueprint, runbook, or customer hand-off is updated at the same time.
