...
This architecture page documents the `blueprints/extensions/waf` deployment. It is intentionally text-first and ASCII-only so it can be reviewed in terminals, pull requests, and customer notes without a diagramming tool.

## Deployment Purpose

This blueprint provides optional Web Application Firewall policy and load-balancer
...
|  01. resource oci_waf_web_app_firewall_policy.this             (main.tf)                     |
|  02. resource oci_waf_web_app_firewall.this                    (main.tf)                     |
+----------------------------------------------------------------------------------------------+
|   |                                                                                          |
|   v                                                                                          |
+----------------------------------------------------------------------------------------------+
...
- `resource oci_waf_web_app_firewall_policy.this` in `main.tf`: creates a WAF policy.
- `resource oci_waf_web_app_firewall.this` in `main.tf`: attaches a WAF policy to the protected backend.
...
resource_ids = { ... }
waf_policy_id = "ocid1.<resource>..."
web_app_firewall_id = "ocid1.<resource>..."
```
...
For WAF, the important hand-off values are `blueprint_name`, `name_prefix`, `resource_ids`, `waf_policy_id`, `web_app_firewall_id`. Keep those names stable unless a downstream blueprint, runbook, or customer hand-off is updated at the same time.
