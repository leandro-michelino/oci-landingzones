...
output "secondary_cluster" {
  description = "Secondary-cluster hand-off object for AWS EKS, including cluster identity fields required for standby and failover operations."
  value = {
    cloud        = "aws"
...
output "dns_failover_contract" {
  description = "DNS failover metadata for OCI primary to AWS standby cutover, including OCI Traffic Management resource IDs when enabled."
  value       = var.enable_traffic_steering_contract ? local.dns_failover_contract : null
}
...
output "operator_summary" {
  ...
  value = {
    ...
    kubernetes = {
      guidance    = "Use the latest common Kubernetes minor version supported by OKE and EKS in the selected regions."
      oke_version = var.oke_kubernetes_version
      eks_version = "Set by the AWS CloudFormation KubernetesVersion parameter; this E2E run used 1.35 because eu-west-1 did not expose 1.36."
    }
    resource_names = {
    ...
    traffic_management = {
      ...
      failover_order     = "OCI primary, AWS standby"
    ...
    useful_commands = {
      ...
      eks_kubeconfig = var.eks_cluster_name == null ? null : "aws eks update-kubeconfig --region <aws-region> --name ${var.eks_cluster_name}"
