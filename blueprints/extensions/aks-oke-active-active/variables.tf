...
variable "interconnect_mode" {
  ...
  type        = string
  default     = "expressroute-fastconnect-partner"

  validation {
    condition     = var.interconnect_mode == "expressroute-fastconnect-partner"
    error_message = "interconnect_mode must be expressroute-fastconnect-partner."
  }
}

variable "fastconnect_virtual_circuit_id" {
  description = "OCI FastConnect virtual circuit OCID used by the Azure partner interconnect path."
  type        = string
  default     = null
}

variable "expressroute_circuit_id" {
  description = "Azure ExpressRoute circuit resource ID paired with FastConnect through the partner."
  type        = string
  default     = null
}
...
variable "aks_cluster_id" {
  description = "Existing AKS cluster resource ID used as the secondary active/active target."
  type        = string
  default     = null
}

variable "aks_cluster_name" {
  description = "Existing AKS cluster name used in hand-off outputs."
  type        = string
  default     = null
}

variable "aks_resource_group_name" {
  description = "Azure resource group name that owns the AKS cluster."
  type        = string
  default     = null
}
...
variable "gitops_tool" {
  description = "GitOps controller used for active/active deployment orchestration."
  type        = string
  default     = "argocd"
...
variable "enable_traffic_steering_contract" {
  description = "Publish traffic steering contract metadata for DNS or global traffic management."
  type        = bool
  default     = true
}

variable "app_fqdn" {
  description = "Application FQDN used for cross-cloud traffic steering decisions."
  type        = string
...
variable "azure_secondary_endpoint" {
  description = "Secondary endpoint for Azure-hosted ingress or gateway."
  type        = string
  default     = null
}

variable "oci_primary_traffic_percent" {
  description = "Traffic percentage directed to OCI as primary. Remaining traffic goes to AKS secondary."
  type        = number
  default     = 100
