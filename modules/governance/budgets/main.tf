...
resource "oci_budget_budget" "this" {
  ...
  display_name                          = coalesce(each.value.display_name, "${local.name_prefix}-bgt-${each.key}")
...
resource "oci_budget_alert_rule" "this" {
  ...
  display_name   = coalesce(each.value.display_name, "${local.name_prefix}-bgtal-${each.value.budget_key}-${each.value.rule_key}")
