...
Available patterns should have a self-contained blueprint folder with its own
...
customer review.
Status `Available` means the folder has deployable foundation wiring, even
...
| --- | --- | --- | --- |
| Core | Core landing zone with no application networking | `blueprints/core/` | Available |
| CIS | CIS Level 1 landing zone | `blueprints/cis/level1/` | Available |
| CIS | CIS Level 2 landing zone | `blueprints/cis/level2/` | Available |
| Identity | CIS basic identity baseline | `blueprints/identity/cis-basic/` | Available |
| Identity | New identity domain | `blueprints/identity/new-identity-domain/` | Available |
| Identity | Custom identity domain | `blueprints/identity/custom-identity-domain/` | Available |
| Identity | Azure Entra to OCI IAM federation baseline | `blueprints/identity/azure-entra-federation/` | Planned |
| Operating entity | Single operating entity onboarding | `blueprints/operating-entity/` | Available |
| Operating entity | Multi-operating-entity landing zone | `blueprints/operating-entity/multi-operating-entities/` | Available |
| Operating entity | Workload vending / application team onboarding | `blueprints/operating-entity/workload-vending/` | Available |
| Networking | Standalone three-tier VCN defaults | `blueprints/networking/standalone-three-tier-vcn-defaults/` | Available |
| Networking | Standalone three-tier VCN custom | `blueprints/networking/standalone-three-tier-vcn-custom/` | Available |
| Networking | Standalone three-tier VCN with ZPR | `blueprints/networking/standalone-three-tier-vcn-zpr/` | Available |
| Networking | Standalone private endpoint only | `blueprints/networking/standalone-private-endpoint-only/` | Available |
| Networking | Externally managed VCNs | `blueprints/networking/externally-managed-vcns/` | Available |
| Networking | Hub-spoke with DRG and three-tier VCNs | `blueprints/networking/hub-spoke-with-drg-and-three-tier-vcns/` | Available |
| Networking | Hub-spoke with IPSec VPN | `blueprints/networking/hub-spoke-with-hub-vcn-ipsec-vpn/` | Available |
...
| Networking | Hub-spoke with ZPR micro-segmentation | `blueprints/networking/hub-spoke-with-zpr-micro-segmentation/` | Available |
| Networking | Hub-spoke with dual-region DR | `blueprints/networking/hub-spoke-with-dual-region-dr/` | Available |
...
| Compliance | Zero Trust landing zone | `blueprints/compliance/zero-trust/` | Available |
| Compliance | Healthcare / PCI compliance landing zone | `blueprints/compliance/healthcare-pci/` | Available |
| Compliance | Security posture automation (Cloud Guard + Vuln Scan + auto-remediation) | `blueprints/compliance/security-posture/` | Available |
| Disaster recovery | Full Stack Disaster Recovery | `blueprints/disaster-recovery/fsdr/` | Available |
| Disaster recovery | Azure + OCI cross-cloud DR | `blueprints/disaster-recovery/azure-oci-cross-cloud-dr/` | Available |
| Disaster recovery | AWS + OCI cross-cloud DR | `blueprints/disaster-recovery/aws-oci-cross-cloud-dr/` | Available |
| Data platform | Autonomous Database ATP / ADW | `blueprints/data-platform/autonomous-database/` | Available |
...
| AI | OCI AI Services (Vision, Language, Speech, Document Understanding, Anomaly Detection) | `blueprints/ai/ai-services/` | Available |
| AI | GenAI multi-model gateway (API Gateway routing, per-team quotas, cost tagging, audit log) | `blueprints/ai/genai-gateway/` | Available |
| AI | Azure + OCI AI gateway (Azure OpenAI + OCI Generative AI routing by region, cost, and data residency) | `blueprints/ai/azure-oci-ai-gateway/` | Available |
| AI | GenAI fine-tuning and dedicated AI cluster (training data bucket, custom model endpoint) | `blueprints/ai/genai-fine-tuning/` | Available |
| AI | GenAI guardrails and observability (PII redaction, token alarms, audit trail, Cloud Guard) | `blueprints/ai/genai-guardrails/` | Available |
| AI | Document intelligence pipeline (Document Understanding + GenAI, intake/output buckets) | `blueprints/ai/document-intelligence/` | Available |
| AI | Embedding and vector ingestion pipeline (chunking, GenAI embedding, OpenSearch index) | `blueprints/ai/embedding-pipeline/` | Available |
| AI | Multi-agent orchestration (orchestrator + specialist agents, Streaming, tool registry, audit) | `blueprints/ai/multi-agent/` | Available |
| AI | AI Agents - RAG landing zone (agents, knowledge base, OpenSearch) | `blueprints/ai/agents/` | Available |
| DevOps | OCI DevOps pipeline | `blueprints/devops/oci-devops-pipeline/` | Available |
...
| Extensions | Oracle Analytics Cloud | `blueprints/extensions/oac/` | Available |
| Extensions | Oracle Integration Cloud | `blueprints/extensions/oic/` | Available; includes sample tfvars and a real lifecycle test runner with identity-domain and entitlement prerequisites |
| Extensions | Observability platform | `blueprints/extensions/observability/` | Available |
| Extensions | Streaming extension | `blueprints/extensions/streaming/` | Available |
| Extensions | WAF extension | `blueprints/extensions/waf/` | Available |
...
| Extensions | Oracle Digital Assistant | `blueprints/extensions/digital-assistant/` | Available |
...
| Data platform | OCI NoSQL Database (key-value / document store, optional app network, IAM, replica) | `blueprints/data-platform/nosql/` | Available; includes sample tfvars and a real lifecycle test runner |
